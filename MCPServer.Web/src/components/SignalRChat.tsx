...
const SignalRChat: React.FC = () => {
  const [connection, setConnection] = useState<signalR.HubConnection | null>(null);
  const [messages, setMessages] = useState<ChatMessage[]>([]);
  const [input, setInput] = useState<string>('');
  const [loading, setLoading] = useState<boolean>(false);
  const [sessionId, setSessionId] = useState<string>('web-session-' + Math.random().toString(36).substring(2, 9));
  ...
  useEffect(() => {
    ...
    newConnection.on('ReceiveMessage', (response: SignalRResponse) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await connection.invoke('SendMessage', chatRequest);
