...
foreach ($file in $serviceFiles) {
    ...
    if ($usingsToAdd.Count -gt 0) {
        ...
        # Find the position to insert the using statements (after the last using statement)
        $lastUsingMatch = [regex]::Match($content, "using [^;]+;")
        $lastUsingPosition = 0
        
        while ($lastUsingMatch.Success) {
            $lastUsingPosition = $lastUsingMatch.Index + $lastUsingMatch.Length
            $lastUsingMatch = $lastUsingMatch.NextMatch()
        }
        
        if ($lastUsingPosition -gt 0) {
            # Find the next newline after the last using statement
            $nextNewlinePosition = $content.IndexOf("`n", $lastUsingPosition)
            if ($nextNewlinePosition -ge 0) {
                $insertPosition = $nextNewlinePosition + 1
                $usingsText = [string]::Join("`r`n", $usingsToAdd) + "`r`n"
                $content = $content.Substring(0, $insertPosition) + $usingsText + $content.Substring($insertPosition)
            }
        }
    }
