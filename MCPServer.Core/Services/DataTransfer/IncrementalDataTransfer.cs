...
namespace MCPServer.Core.Services.DataTransfer
{
    public class IncrementalDataTransfer
    {
            ...
            string timestampColumnName)
        {
                ...
                using (var connection = new SqlConnection(_destinationConnectionString))
                {
                    ...
                    string checkSql = @"
                        SELECT CASE WHEN EXISTS (
                            SELECT * FROM sys.tables WHERE name = 'IncrementalLoadWatermarks' AND schema_id = SCHEMA_ID('dbo')
                        ) THEN 1 ELSE 0 END";
                    ...
                    using (var checkCommand = new SqlCommand(checkSql, connection))
                    {
                        ...
                        tableExists = Convert.ToBoolean(result);
                    }

                    if (tableExists)
                    {
                        // If table exists, try to get the watermark
                        string sql = @"
            ...
            {
                LogError($"Database error getting last timestamp: {ex.Message}. Starting with full load.", ex);
                // Continue with null timestamp (full load) if there's an error
            }
            catch (Exception ex)
            {
                LogError($"Error getting last timestamp: {ex.Message}. Starting with full load.", ex);
                // Continue with null timestamp (full load) if there's an error
            }
            ...
            DateTime timestamp)
        {
            try
            {
                using (var connection = new SqlConnection(_destinationConnectionString))
                {
                    ...
                    if (!tableExists)
                    {
                        ...
                        if (!tableExists)
                        {
                            LogInfo("Skipping watermark update as table could not be created");
                            return;
                        }
                    }
            ...
            {
                LogError($"Database error updating watermark: {ex.Message}. Continuing without updating watermark.", ex);
                // Continue without updating watermark
            }
            catch (Exception ex)
            {
                LogError($"Error updating watermark: {ex.Message}. Continuing without updating watermark.", ex);
                // Continue without updating watermark
            }
        }
        ...
        private async Task EnsureWatermarkTableExistsAsync(SqlConnection connection)
        {
            try
            {
                ...
                if (!tableExists)
                {
                    LogInfo("Creating watermark tracking table in destination database");
            ...
            {
                // Log the error but don't throw - we'll continue without watermarks
                LogError($"Error creating watermark table: {ex.Message}. Continuing without watermark tracking.", ex);
            }
            catch (Exception ex)
            {
                // Log the error but don't throw - we'll continue without watermarks
                LogError($"Unexpected error creating watermark table: {ex.Message}. Continuing without watermark tracking.", ex);
            }
        }
